...
class ChunkVertex
{
  public:
    ...
    constexpr ChunkVertex(uint8_t offsetX, uint8_t offsetY, uint8_t offsetZ,
                          uint8_t u, uint8_t v, BlockFace blockFace)
        : m_Encoding(offsetX | (offsetY << 6u) | (offsetZ << 12u) | (u << 26u) |
                     (v << 27u) | (static_cast<uint8_t>(blockFace) << 28u))
    {
    }

    LocalBlockCoords GetLocalCoords() const
    {
        return {static_cast<uint8_t>(m_Encoding & 0x3Fu),

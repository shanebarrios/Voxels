...
{
	static BufferLayout layout{ {LayoutElementType::UInt, 1}, {LayoutElementType::UInt, 1} };
	return layout;
} 

void ChunkVertex::Offset(uint8_t x, uint8_t y, uint8_t z)
{
	assert(x <= CHUNK_DIMENSION && y <= CHUNK_DIMENSION && z <= CHUNK_DIMENSION && "Position out of bounds!");
	m_Encoding += 
		static_cast<size_t>(x) | 
		(static_cast<size_t>(y) << 6u) | 

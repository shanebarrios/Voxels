...
void PlayerController::SetPhysicsEnabled(bool yes)
{
    ...
    {
        m_PhysicsComponent = &m_ECS.AddComponent<PhysicsComponent>(
            m_Player, EntityFactory::CreatePlayerPhysicsComponent());
    }
...
void PlayerController::HandleBlockInteractions(const Camera& camera)
{
    ...
    const WorldCoords lookDirection = {
        std::cosf(MathUtils::DegsToRadians(m_LookComponent.Yaw)) *
            std::cosf(MathUtils::DegsToRadians(m_LookComponent.Pitch)),
        std::sinf(MathUtils::DegsToRadians(m_LookComponent.Pitch)),
        std::sinf(MathUtils::DegsToRadians(m_LookComponent.Yaw)) *
            std::cosf(MathUtils::DegsToRadians(m_LookComponent.Pitch))};
    const WorldCoords lookOrigin =
        m_TransformComponent.Position + m_LookComponent.Offset;
    ...
    if (result)
    {
        ...
        {
            const BlockCoords normal =
                ChunkUtils::k_FaceNormals[static_cast<uint8_t>(result->Face)];
            const BlockCoords coords = result->Coords + normal;

            if (!m_PhysicsComponent || !PhysicsUtils::ColliderIntersectsBlock(
                                           coords, m_PhysicsComponent->Collider,

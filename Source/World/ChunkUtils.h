...
{
inline constexpr std::array<BlockCoords, static_cast<size_t>(BlockFace::Count)>
    k_FaceNormals{{// Front
                   {0, 0, 1},
                   // Back
                   {0, 0, -1},
...
inline size_t NeighboringChunks(ChunkCoords chunkCoords, uint8_t blockX,
                                uint8_t blockY, uint8_t blockZ,
                                std::array<ChunkCoords, 3>& neighbors)
{
    size_t i = 0;
    if (blockX == 0)
    {
...
inline constexpr bool BlockInChunkBounds(BlockCoords blockCoords,
...
{
    return blockCoords.X >= 0 && blockCoords.X < CHUNK_DIMENSION &&
...
{
    return {ExtractX(index), ExtractY(index), ExtractZ(index)};
}

...
struct ChunkCoords
{
    ...
    explicit operator ChunkCoords2D() const { return {X, Z}; }
    ...
    constexpr ChunkCoords operator*(int k) { return {X * k, Y * k, Z * k}; }
...
struct LocalBlockCoords2D
{
    ...
    constexpr size_t ToIndex() const
    {
        return static_cast<size_t>(X) | static_cast<size_t>(Z)
                                            << CHUNK_COORD_BIT_COUNT;
    }
};
...
struct BlockCoords
{
    ...
    {
        return {X >= 0 ? X / CHUNK_DIMENSION
                       : (X - CHUNK_DIMENSION + 1) / CHUNK_DIMENSION,
                Y >= 0 ? Y / CHUNK_DIMENSION
    ...
    {
        return {static_cast<uint8_t>(X & CHUNK_COORD_MASK),
    ...
    constexpr BlockCoords operator*(int k) { return {X * k, Y * k, Z * k}; }
...
struct WorldCoords
{
    ...
    static constexpr WorldCoords Lerp(const WorldCoords& c1,
                                      const WorldCoords& c2, float alpha)
    {
        return {c1.X * (1 - alpha) + c2.X * alpha,
    ...
    explicit operator ChunkCoords() const
    {
        ...
        return {static_cast<int>(std::floor(X)) / intDim,
    ...
    {
        return {k * coords.X, k * coords.Y, k * coords.Z};
    }
...
{
    return {block.X + local.X, block.Y + local.Y, block.Z + local.Z};
}

constexpr BlockCoords operator+(LocalBlockCoords local, BlockCoords block)
{
    return {local.X + block.X, local.Y + block.Y, local.Z + block.Z};
}
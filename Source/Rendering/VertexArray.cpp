...
static size_t GetSizeOfType(LayoutElementType type)
{
    ...
    case LayoutElementType::UByte: return 1;
    case LayoutElementType::Short:
    case LayoutElementType::UShort: return 2;
    case LayoutElementType::Int:
    case LayoutElementType::UInt:
    case LayoutElementType::Float: return 4;
    default: assert(false && "Unsupported layout element type"); return 0;
    }
}
    ...
    for (size_t i = 0; i < elements.size(); i++)
    {
        ...
        if (element.Type == LayoutElementType::Float)
        {
            glVertexAttribPointer(static_cast<GLuint>(i),
                                  ...
                                  static_cast<GLenum>(element.Type), GL_FALSE,
        ...
        else
        {
            glVertexAttribIPointer(
                static_cast<GLuint>(i), static_cast<GLint>(element.Count),
                static_cast<GLenum>(element.Type), static_cast<GLsizei>(stride),
                reinterpret_cast<const GLvoid*>(offset));
        }

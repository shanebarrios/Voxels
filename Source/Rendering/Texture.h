...
class Texture2D : public Texture
{
  public:
    ...
    static Texture2D FromPath(
        std::string_view path,
        TextureWrap wrapMethod = TextureWrap::ClampToEdge);

    static Texture2D FromData(
        const void* data, int width, int height, TextureInternalFormat format,
        TextureWrap wrapMethod = TextureWrap::ClampToEdge);
  ...
  private:
    explicit Texture2D(uint32_t id, TextureInternalFormat format, int width,
                       int height);
...
//{
// public:
//	static std::shared_ptr<TextureCube> Create(const

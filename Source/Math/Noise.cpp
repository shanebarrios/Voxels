...
struct Vec2
{
    ...
    static constexpr float Dot(Vec2 v1, Vec2 v2)
    {
...
static constexpr std::array<uint8_t, 256> k_Permutation = {
...
static float OctaveNoiseNonNormalized(Noise::OctaveConfig config, float x,
                                      float y)
{
    float accum = 0.0f;
    float amplitude = 1.0f;
    float frequency = config.BaseFrequency;

    for (int i = 0; i < config.NumOctaves; i++)
    {
        const float noise = Noise::PerlinNoise(x * frequency, y * frequency);
        accum += noise * amplitude;
...
float PerlinNoise(float x, float y)
{
    ...
    const Vec2 gradTopLeft =
        GetRandomGradient(permutation[permutation[xi] + yi + 1]);
    const Vec2 gradTopRight =
        GetRandomGradient(permutation[permutation[xi + 1] + yi + 1]);
    const Vec2 gradBottomLeft =
        GetRandomGradient(permutation[permutation[xi] + yi]);
    const Vec2 gradBottomRight =
        GetRandomGradient(permutation[permutation[xi + 1] + yi]);
    ...
    return MathUtils::Lerp(MathUtils::Lerp(influence3, influence4, u),
                           MathUtils::Lerp(influence1, influence2, u), v) *
           SQRT2;
}
...
void OctavePerlinNoise::EstimateMinMax()
{
    OctaveConfig adjustedConfig = m_Config;
    adjustedConfig.BaseFrequency = 1.0f;
    for (float y = 0.0f; y < 256.0f; y += 0.213f)
    {
        for (float x = 0.0f; x < 256.0f; x += 0.178f)
        {
            const float result = OctaveNoiseNonNormalized(adjustedConfig, x, y);
            if (result < m_MinEstimate)
                m_MinEstimate = result;
            if (result > m_MaxEstimate)
                m_MaxEstimate = result;
        }
    }
}

float OctavePerlinNoise::Sample(float x, float y) const
{
    const float nonNormalized = OctaveNoiseNonNormalized(m_Config, x, y);
    const float normalized =
        (nonNormalized - m_MinEstimate) / (m_MaxEstimate - m_MinEstimate);

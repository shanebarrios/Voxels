...
static float OctaveNoiseNonNormalized(Noise::OctaveConfig config, float x, float y)
{
    float accum = 0.0f;
    ...
    void OctavePerlinNoise::EstimateMinMax()
    {
        ...
        for (float y = 0.0f; y < 256.0f; y+=0.213f)
        {
            for (float x = 0.0f; x < 256.0f; x+=0.178f)
            {
                ...
                if (result < m_MinEstimate) m_MinEstimate = result;
                if (result > m_MaxEstimate) m_MaxEstimate = result;
            }
        }
    }

...
Application::Application() :
	...
	m_World{},
	m_Camera{m_World.GetPlayerView()},
	m_Renderer{Config::WindowWidth, Config::WindowHeight},
	m_UIOverlay{}
{
	s_Instance = this;
	Logger::Init();
...
void Application::Run()
{
	...
	while (m_Running)
	{
		...
		const float deltaTime = duration<float>(currentFrame - lastFrame).count();
...
{
	if (event.Action == KeyAction::Press && (event.Code == KeyCode::Esc || m_UIOverlay.IsVisible()))
	{
		Input::Reset();

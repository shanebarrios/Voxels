...
{
inline BlockCoords ColliderBlocksMinBounds(const Collider& collider,
                                           WorldCoords position)
{
    return static_cast<BlockCoords>(WorldCoords{position.X + collider.Min.X,
                                                position.Y + collider.Min.Y,
                                                position.Z + collider.Min.Z});
}

inline BlockCoords ColliderBlocksMaxBounds(const Collider& collider,
                                           WorldCoords position)
{
    return static_cast<BlockCoords>(WorldCoords{position.X + collider.Max.X,
                                                position.Y + collider.Max.Y,
                                                position.Z + collider.Max.Z});
}

inline bool ColliderIntersectsBlock(BlockCoords blockCoords,

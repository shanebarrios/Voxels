...
void UIOverlay::Init(Window* window, Camera* camera, World* world)
{
    m_Window = window;
    m_Camera = camera;
    m_World = world;
...
void UIOverlay::BeginRender()
{
    ...
    {
        const WorldCoords playerPos =
            m_World->GetPlayerView().Transform->Position;
        const ChunkCoords playerPosChunk = static_cast<ChunkCoords>(playerPos);
        ImGui::Text("World pos: (%f, %f, %f)", playerPos.X, playerPos.Y,
                    playerPos.Z);
        ImGui::Text("Chunk pos: (%d, %d, %d)", playerPosChunk.X,
                    playerPosChunk.Y, playerPosChunk.Z);
        ...
        if (ImGui::Combo("Active block", &option, blocks,
                         static_cast<int>(BlockType::NumBlockTypes) - 1))
        {
